"""Tests for ``downloader/url_resolver.py`` media URL helpers.

Covers both targeted unit/edge-case tests and a property-based test for
Property 14 (media-URL normalization and dedupe). Hypothesis is used when
available; otherwise a deterministic random-generation fallback exercises the
same property across many inputs.
"""
...
class DedupeMediaUrlsTests(unittest.TestCase):
    ...
    def test_dedupes_after_normalization(self) -> None:
        candidates = [
            "  https://host/a.mp4 ",
            "//host/a.mp4",  # normalizes to the same https URL
            "https://host/a.mp4",
        ]
        self.assertEqual(dedupe_media_urls(candidates), ["https://host/a.mp4"])
...
class Property14MediaUrlNormalizationAndDedupe(unittest.TestCase):
    """Property 14: media-URL normalization and dedupe.

    The resolved URL list equals the order-preserving deduplication of the
    candidates that normalize to a valid media URL (http/https scheme,
    .mp4/.mkv/.flv extension, OSS snapshot thumbnails rejected); invalid
    ...
    **Validates: Requirements 7.2**
    ...
    def _assert_property(self, candidates: list[str]) -> None:
        ...
        # Every output is a valid normalized media URL.
        for url in result:
            self.assertEqual(normalize_media_url(url), url)
        ...
        # Output equals the order-preserving dedupe of normalized-valid inputs.
        expected: list[str] = []
        seen: set[str] = set()
        ...
        # Invalid candidates never survive.
        valid_normalized = {
