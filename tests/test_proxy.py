"""Tests for proxy resolution (``config/proxy.py``).

Covers targeted unit/edge-case tests plus a property-based test for
Property 26 (proxy precedence): *for any* combination of environment proxy
variable and system proxy setting, ``resolve_proxy`` returns the environment
proxy when present, otherwise the system proxy when present, otherwise
``None``.

Both the environment lookup and the system-proxy lookup are injected so the
precedence logic is exercised without touching the real process environment or
the Windows registry. Hypothesis is used when available; otherwise a
deterministic random-generation fallback exercises the same property.

**Validates: Requirements 9.6, 9.7, 9.8, 14.5**
"""
...
# A strategy/sample of realistic, non-empty proxy server strings (no leading or
# trailing whitespace, so that resolution returns them verbatim).
_PROXY_SAMPLES = (
...
class ResolveEnvProxyTests(unittest.TestCase):
    """Example/edge-case checks for :func:`resolve_env_proxy`."""
...
class ResolveSystemProxyTests(unittest.TestCase):
    """Checks for the guarded system-proxy lookup."""
    ...
    def test_never_raises(self) -> None:
        # Should resolve to a string or None on any platform without raising.
        result = resolve_system_proxy()
        self.assertTrue(result is None or isinstance(result, str))


class ResolveProxyPrecedenceTests(unittest.TestCase):
    """Example-based precedence checks for :func:`resolve_proxy`."""
...
def _expected(env_proxy: str | None, system_proxy: str | None) -> str | None:
    """Reference implementation of the precedence rule for assertions."""
...
class ProxyPrecedenceProperty(unittest.TestCase):
    """Property 26: proxy precedence across all presence combinations."""
    ...
    def test_all_presence_combinations(self) -> None:
        # Exhaustively cover the four presence combinations explicitly.
