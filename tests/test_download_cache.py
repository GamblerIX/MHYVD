"""Tests for ``cache/download_cache.py`` (Download_Cache persistence).

Covers targeted unit/edge-case tests plus a property-based test for Property 23
(download-cache round-trip). Hypothesis is used when available; otherwise a
...
class DownloadCacheUnitTests(unittest.TestCase):
    ...
    def test_add_persists_immediately(self) -> None:
        ...
        # The file exists and contains the record without a second save call.
    ...
    def test_unexpected_shape_loads_empty(self) -> None:
        # A JSON array is valid JSON but the wrong shape for a keyed cache.
...
class Property23DownloadCacheRoundTrip(unittest.TestCase):
    """Property 23: download-cache round-trip.

    *For any* set of download records (article URL, resolved video URL, file
    path, title, category, file size), adding them to a ``DownloadCache`` and
    then loading a fresh ``DownloadCache`` from the same file preserves all
    record fields; a missing or corrupt cache file loads as an empty cache
    without raising.

    **Validates: Requirements 8.4, 8.5, 8.6, 14.4**
    """
    ...
    def _assert_round_trip(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Expected state: last write wins per article URL.
            ...
            # A freshly loaded cache reflects exactly the persisted records.
    ...
    if _HAS_HYPOTHESIS:
        ...
        _record = st.tuples(
            st.text(min_size=1, max_size=40),  # article_url (the key)
            _text,  # video_url
            _text,  # file_path
            _text,  # title
            _text,  # category
            st.integers(min_value=0, max_value=10**15),  # file_size
        )
