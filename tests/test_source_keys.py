"""Tests for source-key composition and parsing (``sources/keys.py``).

Covers Property 2 (Source_Key round-trip) from the design document:
*for any* game key and region key,
``parse_source_key(make_source_key(game, region)) == (game, region)``,
and the produced key contains both the game and the region.
"""
...
class TestMakeSourceKey(unittest.TestCase):
    """Example-based checks for :func:`make_source_key`."""
...
class TestParseSourceKey(unittest.TestCase):
    """Example-based checks for :func:`parse_source_key`."""
    ...
    def test_region_may_contain_separator(self) -> None:
        # Only the first separator delimits game from region.
...
class TestSourceKeyRoundTrip(unittest.TestCase):
    """Property 2: Source_Key round-trip."""

    # Game keys never contain the separator (design invariant); region keys
    # may. Both constraints keep generated values within the valid input space.
    ...
    def test_round_trip_over_many_inputs(self) -> None:
        ...
        for _ in range(2000):
            ...
            with self.subTest(game=game, region=region):
                ...
                # The composed key contains both components.
