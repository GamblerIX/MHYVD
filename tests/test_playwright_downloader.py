...
class DownloadFileBlockingCleanupTests(unittest.TestCase):
    def test_part_file_removed_when_download_raises(self) -> None:
        with tempfile.TemporaryDirectory() as out:
            dl = PlaywrightDownloader(output_dir=out)
            ...
            with mock.patch("urllib.request.urlopen", return_value=response):
                with self.assertRaises(OSError):
                    dl._download_file_blocking("https://cdn/clip.mp4", target)

            part = target.with_suffix(f"{target.suffix}.part")
            self.assertFalse(part.exists(), ".part file leaked after failure")
...
class PropertyTests(unittest.TestCase):
    ...
    def test_property_19_resume_skips_existing(self, item_count: int) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            async def resolve(driver, item):  # pragma: no cover
...
class DiskSpaceGateTests(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        self.out = Path(self._tmp.name)
    ...
    def _downloader(self, free_bytes_or_probe) -> PlaywrightDownloader:
        resolve_calls: list[str] = []
        self.resolve_calls = resolve_calls

        async def resolve(driver, item):
            resolve_calls.append(item.url)
            return "https://cdn/clip.mp4"
        ...
        probe = (
            free_bytes_or_probe
            if callable(free_bytes_or_probe)
            else (lambda directory: free_bytes_or_probe)
        )
    ...
    def _run_one(self, dl: PlaywrightDownloader) -> DownloadResult:
        grouped = {"videos/pv": make_items("videos/pv", 1)}
        [result] = run(dl.download(grouped, _Driver()))
        return result

    def test_insufficient_space_fails_without_resolving(self) -> None:
        dl = self._downloader(MIN_FREE_BYTES - 1)
        grouped = {"videos/pv": make_items("videos/pv", 2)}
        results = run(dl.download(grouped, _Driver()))
        self.assertEqual([r.status for r in results], [STATUS_FAILED] * 2)
        for r in results:
            self.assertIn("Insufficient disk space", r.error or "")

        self.assertEqual(self.resolve_calls, [])

    def test_exactly_threshold_is_insufficient(self) -> None:

        dl = self._downloader(MIN_FREE_BYTES)
        self.assertEqual(self._run_one(dl).status, STATUS_FAILED)

    def test_sufficient_space_downloads(self) -> None:
        dl = self._downloader(MIN_FREE_BYTES + 1)
        result = self._run_one(dl)
        self.assertEqual(result.status, STATUS_DOWNLOADED)
        self.assertEqual(self.resolve_calls, [result.url])

    def test_probe_failure_does_not_block(self) -> None:
        def broken_probe(directory: Path) -> int:
            raise OSError("statvfs failed")

        dl = self._downloader(broken_probe)
        self.assertEqual(self._run_one(dl).status, STATUS_DOWNLOADED)

    def test_default_threshold_is_5_gib(self) -> None:
        self.assertEqual(MIN_FREE_BYTES, 5 * 1024**3)
        self.assertEqual(PlaywrightDownloader().min_free_bytes, MIN_FREE_BYTES)


class PostDownloadHookTests(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
    ...
    def test_hook_called_per_successful_download(self) -> None:
        async def resolve(driver, item):
            return f"https://cdn/{item.title}.mp4"
        ...
        self.assertEqual(sorted(seen), ["Title 0", "Title 1"])
        self.assertTrue(all(not r.local_path.exists() for r in results))

    def test_hook_not_called_for_failed_download(self) -> None:
        async def resolve(driver, item):
            return None
    ...
    def test_hook_result_replaces_download_result(self) -> None:
        async def resolve(driver, item):
            return "https://cdn/clip.mp4"
        ...
        dl = PlaywrightDownloader(
            ...
            resolve_attempt=resolve,
            file_downloader=self._writer(),
            post_download=lambda r: dataclasses.replace(r, error="upload failed"),
        )
        grouped = {"videos/pv": make_items("videos/pv", 1)}
        [result] = run(dl.download(grouped, _Driver()))
...
class RemoteExistsSkipTests(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self._tmp.cleanup)
        self.out = Path(self._tmp.name)

    def _downloader(self, remote_exists) -> PlaywrightDownloader:
        async def resolve(driver, item):
    ...
    def test_remote_hit_skips_download(self) -> None:
        probed: list[Path] = []

        def remote_exists(relative: Path) -> bool:
            probed.append(relative)
            return relative.suffix == ".mp4"

        dl = self._downloader(remote_exists)
        grouped = {"videos/pv": make_items("videos/pv", 1)}
        [result] = run(dl.download(grouped, _Driver()))

        self.assertEqual(result.status, STATUS_SKIPPED)
        self.assertFalse(any(self.out.rglob("*.mp4")))
        self.assertTrue(all(not p.is_absolute() for p in probed))

    def test_remote_miss_downloads(self) -> None:
        dl = self._downloader(lambda relative: False)
        grouped = {"videos/pv": make_items("videos/pv", 1)}
        [result] = run(dl.download(grouped, _Driver()))

        self.assertEqual(result.status, STATUS_DOWNLOADED)

    def test_probe_error_falls_through_to_download(self) -> None:
        def remote_exists(relative: Path) -> bool:
    ...
    def test_without_remote_exists_no_probe(self) -> None:
        dl = self._downloader(None)
        grouped = {"videos/pv": make_items("videos/pv", 1)}
        [result] = run(dl.download(grouped, _Driver()))
