...
依赖由 `pyproject.toml` 声明，使用 uv 管理。Python 版本固定为 3.11 。
...
> 浏览器相关的依赖采用延迟导入的方式，因此在未安装真实浏览器的情况下，配置、分类器和流水线的单元测试仍可正常运行。
...
| `-o, --timeout SECONDS` | 抓取列表阶段的时间预算 / 单次操作超时时长（秒），下载视频的用时不计入预算。会覆盖配置文件中的 `timeout` 设置。 |
| `--headless` / `--headed` | 浏览器运行模式（互斥参数；默认为有头 headed 模式，CI/服务器环境请使用 `--headless`）。 |
...
| `--list-only` | 仅抓取、分类并导出 URL 列表（`<output_dir>/cache.json`），不下载视频。与 `--from-release` 互斥。 |
| `--from-release [URL]` | 跳过浏览器抓取列表：自动下载 GitHub `url-list` Release 发布的 `url-list.json`（不带值时使用本仓库的默认地址，也可指定任意 URL），然后照常分类并下载其中的视频。会覆盖 `--source`，与 `--list-only` 互斥。 |
...
# 示例：直接使用每日定时任务发布到 Release 的 URL 列表抓取视频（无需重新爬取列表页）
uv run python -m src.main run --from-release --resume
...
timeout: 300          # 单次操作的超时时间（秒）；同时也是抓取列表阶段的时间预算（下载不计入）
rules:
